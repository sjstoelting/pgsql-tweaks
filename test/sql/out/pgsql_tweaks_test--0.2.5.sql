...
WITH test AS
	(
		SELECT is_integer('123') AS isinteger, 0 AS zero
	)
SELECT
	CASE
		WHEN isinteger THEN
...
WITH test AS
	(
		SELECT is_integer('123.456') AS isinteger, 0 AS zero
	)
SELECT
	CASE
...
WITH test AS
	(
		SELECT is_integer('3243546343') AS isinteger, 0 AS zero
	)
SELECT
	CASE
		WHEN NOT isinteger THEN
...
WITH test AS
	(
		SELECT is_smallint('123') AS issmallint, 0 AS zero
	)
SELECT
	CASE
...
WITH test AS
	(
		SELECT is_smallint('123.456') AS issmallint, 0 AS zero
	)
SELECT
	CASE
...
WITH test AS
	(
		SELECT is_smallint('3243546343') AS issmallint, 0 AS zero
	)
SELECT
	CASE
		WHEN NOT issmallint THEN
...
WITH test AS
	(
		SELECT is_json('Not a JSON') AS isjson, 0 AS zero
	)
SELECT
	CASE
...
WITH test AS
	(
		SELECT is_jsonb('Not a JSONB') AS isjsonb, 0 AS zero
	)
SELECT
	CASE
...
SELECT 'Test starting: aggregate_function_gab_fill' AS next_test;
/**
